...
# (2026-09-21 신고: "의미가 통하는 데까지 끊어야 하는데 이상하게 잘린다" — 실측에서
# '우리 / 교회를', '이런 / 식으로', '굉장히 / 가까우신'처럼 꾸미는 말과 꾸밈받는 말이
# 갈라졌다. 관형사·부사를 대폭 보강했다.)
_NO_BREAK_AFTER = {"안", "못", "잘", "더", "덜", "꼭", "왜", "그", "이", "저", "한", "두", "세", "네",
                   "수", "것", "거", "줄", "때", "뿐", "채", "지",
                   # 관형사·대명사 수식(뒤 명사와 한 몸)
                   "우리", "저희", "그런", "이런", "저런", "어떤", "무슨", "웬", "새", "첫", "매",
                   "각", "온", "온갖", "여러", "모든", "다른", "같은", "옛", "지난", "다음", "본",
                   "몇", "여러분", "그의", "저의", "나의", "내", "네", "제",
                   # 뒤 용언을 꾸미는 부사(끊으면 서술어와 갈라진다)
                   "아주", "매우", "굉장히", "너무", "정말", "진짜", "참", "좀", "조금", "많이",
                   "가장", "제일", "늘", "항상", "자주", "다시", "함께", "서로", "빨리", "천천히",
                   "결국", "드디어", "마침내", "거의", "전혀", "절대", "결코", "반드시", "꼭꼭",
                   "이미", "벌써", "아직", "먼저", "바로", "그냥", "막", "갓", "딱", "훨씬"}
# 이 말들 '앞'에서 끊으면 자연스럽다 — 새 문장·새 절을 여는 접속 부사.
_BREAK_BEFORE_BONUS = ("그래서", "그리고", "그러나", "그런데", "하지만", "그러니까", "그러면",
                       "그러므로", "따라서", "왜냐하면", "그럼", "근데", "또한", "즉", "결국")
...
_NO_BREAK_BEFORE_EXACT = {"수", "것", "거", "줄", "때", "뿐", "채", "만큼", "중", "등",
                          ...
                          # 의존명사에 조사가 붙은 꼴도 앞말과 한 몸이다('가까우신 / 거로' 방지)
                          "거로", "거를", "거가", "거는", "거도", "거에", "건", "걸", "게",
                          "것을", "것이", "것은", "것도", "것과", "것에", "것까지",
                          "때가", "때는", "때를", "때에", "때도", "뿐만", "채로", "중에", "중에서",
                          "만큼은", "만큼의", "듯이", "듯한", "척", "법", "리가", "따름입니다",
                          # 관형형 뒤 사람 의존명사('보내신 / 분이', '뿌리는 / 자가')
                          "자", "자가", "자는", "자를", "자도", "자의", "자예요", "자입니다",
                          "분", "분이", "분은", "분을", "분도", "분의", "분들", "분들이",
                          "이", "이가", "이는", "이를"}
...
def _break_score(words: list[Word], j: int, max_gap: float) -> float:
    ...
    if nxt_bare in _NO_BREAK_BEFORE_EXACT or nxt_bare.startswith(_NO_BREAK_BEFORE_PREFIX):
        return -100.0
    score = 0.0
    if prev_bare.endswith(("해야", "돼야", "되야", "어야", "아야", "하여야")):
        score -= 8.0  # '해야 / 할', '믿어야 / 됩니다' — 의무 표현은 뒤 용언과 한 몸
    if nxt_bare in _BREAK_BEFORE_BONUS:
        score += 5.0  # 접속 부사 앞 — 새 호흡이 시작되는 자리
...
def chunk_words_into_lines(
    ...
    while i < n:
        ...
        # 창을 max_words_per_line(+1)로 묶어두면 3~4단어 안에 '끊어도 되는 자리'가 아예
        # 없을 때 억지로 뜻을 갈라야 했다(2026-09-21 신고). 화면 폭(max_units)이 허락하는
        # 한 몇 단어 더 보되, 초과 단어마다 점수를 깎아 '의미 경계를 사는 값'으로만 쓴다.
        for j in range(i + 1, min(i + max_words_per_line + 3, n) + 1):
            ...
            if j >= n:
                ...
                if wlen > max_words_per_line:
                    # 한 단어 초과할 때마다 점점 비싸진다 — 절/문장 끝(+6/+10)이나 금지
                    # 경계 회피(-100)를 얻을 때만 더 길어진다.
                    score -= 2.2 * (wlen - max_words_per_line)
            if score > best_score:
