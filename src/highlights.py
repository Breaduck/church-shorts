...
class Clip:
    ...
    core_score: Optional[float] = None    # 쇼츠 요소(재미·감동·뜨끔·교훈·위로)의 세기 (1~10).
    # ↑ 2026-09-23 통일: 예전엔 여기가 "설교의 핵심 근접도", sermon_cut(v2)은 "쇼츠 요소의 세기"로
    #   서로 다른 뜻이면서 같은 공식(scoring.py)과 같은 등급 뱃지를 썼다. 현행 엔진인 v2 정의로 맞춘다.
...
### 3단계 — 정직한 채점 (너는 세부 축만 매기고, 통합 점수는 시스템이 계산한다)
  - core_score (1~10): **쇼츠 요소의 세기** — 재미(웃음)·감동(실화·희생)·뜨끔(직격)·교훈(관점을 뒤집는 한마디)·
    위로 중 하나가 구체(장면·실화·대사·숫자·질문)와 함께 있고 명제로 착지하는가. 구체 없이 "~입니다" 선언만이면
  ...
  - **hook (1~10)**: hook_line(실제 첫 문장)만 따로 읽고, 스크롤을 멈추는가. 배경 설명·중간을 툭 자른 느낌이면 3 이하
    (4 미만이면 시스템이 viral을 추가로 깎는다 — 그런 후보는 시작점을 옮겨라).
