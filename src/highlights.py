...
- **⛔ 전도지·설교 톤 금지 (이게 제일 흔한 실패, "사이비 같다"는 소리 나오는 원인).** 시청자를 향해 "내 말 들어봐,
  답은 이거야"라고 훈계·전도하는 투로 끝맺지 마라. 특히 아래 "권위 떡밥" 엔딩은 **전부 금지** — 정답을 안 주고
  "성경/하나님이 답을 안다"며 따라오라고만 하는 톤이라 안 믿는 사람에겐 사이비 전단지로 읽힌다:
  - ✗ "…성경은 이렇게 말합니다" / "…성경의 답은 다릅니다" / "…성경은 그렇게 말한 적 없습니다"
  - ✗ "…그것이 기적입니다" / "…그게 바로 은혜입니다" / "…하나님이 함께하십니다" (설교 결론을 그대로 통보)
  - ✗ "당신이 원하는 기적은 무엇입니까?" (추상·막연 질문 — 뭘 묻는지 손에 안 잡혀 아무도 안 궁금해함)
  판별: 제목이 "그래서 클릭하면 답을 알려줄게" 식 예고편이면 실패. **답의 방향(구체적 그림·감정·반전)을 제목 안에서
  이미 살짝 보여줘야** 궁금해서 누른다. 감추기만 하는 건 후킹이 아니라 약 파는 톤이다.
- **⛔ 추상 교회 명사를 훅의 알맹이로 쓰지 마라.** "기적·은혜·축복·사랑·함께하심·회복·소망" 같은 단어는 안 믿는
  사람에겐 아무 그림도 안 떠오르는 공허한 명사다. 반드시 **구체적이고 일상적인 장면·숫자·상황**에 걸어라.
  - ✗ "하나님이 나와 함께 하시는 이야기, 그것이 기적입니다" (추상 명사+통보체, 남 얘기)
    → ✓ "매일 똑같던 하루가 사실 기적이었습니다" / ✓ "아무 일도 안 일어난 게 응답이었어요"
...
- **경건성 절대 규칙 (신성모독·믿음 조롱으로 읽히면 무조건 실패).** 이 채널은 교회 말씀 채널이다. 제목은
  **맥락 없이 그 한 줄만 봐도** 하나님·예수·성경·믿음·기도를 부정하거나 조롱하는 것처럼 읽히면 절대 안 된다.
  ...
  - 통념을 뒤집고 싶으면 **비판 대상을 오해로 명확히** 하되, "성경은 ~"식 권위 떡밥으로 끝내지 마라(위 전도지 톤 금지):
    ✗"예수 믿으면 다 잘된다는 거짓말" / ✗"믿으면 다 잘 풀린다? 성경은 그렇게 말한 적 없습니다"(권위 떡밥·사이비 톤)
    → ✓"예수 믿으면 만사형통? 그거 믿다가 더 지칩니다" / ✓"믿으면 다 잘 풀린다는 말, 저는 안 믿습니다"(반전을 정면으로).
...
- **"주어 + ~앞에선/~해도 + ~습니다/~됩니다" 평서문 통보체 금지.** "OO이라도 OO 앞에선 OO됩니다" 식으로
  주어-서술어만 있고 청자를 향한 질문·도발·긴장이 없는 문장은 설교 결론 요약이지 훅이 아니다(임팩트 없음).
  ✗ "아들이라도 죄 앞에선 버림받습니다" (사실 통보, 남 얘기처럼 읽힘 — 왜 내가 클릭해야 하는지 안 와닿음)
    → ✓ "하나님이 친아들을 버린 순간이 있습니다" (반전 사실을 정면으로) /
      ✓ "죄 앞에선 당신도 예외가 아닙니다" (직접 호명으로 긴장 부여) /
      ✓ "예수님도 버림받았는데, 우리는요?" (질문으로 끝나 클릭 유도)
  판별법: 제목을 소리 내어 읽었을 때 "그래서 뭐?"라는 반응이 나오면 실패 — 듣는 사람을 문장 안으로
  끌어들이는 장치(질문/직접호명/반전/미완의 긴장) 중 하나는 반드시 있어야 한다.
- **나쁜 예 → 좋은 예 (감을 잡아라):**
...
- **title에는 4개(title + 후보3) 중 가장 임팩트 있는 걸 넣어라.** 초안을 그대로 title에 넣고 나머지를
  변주로 채우지 마라 — 4개를 다 쓴 뒤 스스로 "이 중 처음 보는 사람 손가락을 가장 확실히 멈출 건 뭐냐"
  기준으로 다시 순위를 매겨 1등을 title에 배치하라.
...
- **제목·경건성**: title과 후보들을 한 줄씩 떼어 읽어 ①교회 용어·요약체("~는 ~입니다"), ②전도지 톤(권위 떡밥
  "성경은 이렇게 말합니다"류·추상 명사 "그것이 기적입니다"류), ③하나님·예수·성경·믿음을 부정/조롱("거짓말/사기/
  소용없다"류) — 셋 중 하나라도 걸리면 다시 써라. 특히 ②는 "사이비 같다"는 반응의 주범이니 제일 먼저 검사하라.
