...
class Clip:
    ...
    # 이 클립의 장면 유형("재미"/"감동"/"뜨끔")과 실제 첫 문장(전사본 그대로).
...
- **hook_line 검증**: hook_line은 전사본에서 그대로 옮겨 적은 클립 첫 문장이어야 한다. 그 한
  문장만 따로 읽어봐라 — 남의 설교 중간을 툭 자른 느낌이면 시작점을 옮기고, 옮겨도 밋밋하면 hook을 낮게 매겨라.
...
[
  {{
    ...
    "hook_line": "클립 첫 문장을 전사본에서 그대로 옮긴 것 (hook 점수는 이 문장 기준)",
    "core_score": 9,
    "hook": 9,
    "retention": 8,
    "emotion": 8,
