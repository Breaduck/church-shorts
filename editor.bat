...
if not exist "%HERE%venv\Scripts\python.exe" (
  ...
  echo.
  echo [i] setup script finished. run this file again to open the editor.
  pause
  goto :eof
)
...
if errorlevel 1 (
  ...
  echo     run install.bat first, then run  claude  once to log in.
  pause
  goto :eof
)

REM 이미 편집기가 떠 있으면(5000 응답) 브라우저만 연다.
powershell -NoProfile -Command "try { Invoke-WebRequest -Uri 'http://localhost:5000/' -TimeoutSec 2 -UseBasicParsing | Out-Null; exit 0 } catch { exit 1 }"
if not errorlevel 1 (
...
start "church-shorts editor" "%HERE%venv\Scripts\python.exe" -m src.web_app

REM 서버가 뜰 때까지 최대 20초 대기 후 브라우저 열기
for /l %%i in (1,1,20) do (
  powershell -NoProfile -Command "try { Invoke-WebRequest -Uri 'http://localhost:5000/' -TimeoutSec 1 -UseBasicParsing | Out-Null; exit 0 } catch { exit 1 }"
  if not errorlevel 1 (
    start "" "http://localhost:5000/"
